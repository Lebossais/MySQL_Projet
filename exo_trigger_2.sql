...
lock tables banque2.agence write;
INSERT INTO AGENCE (AGE_NOM, AGE_VILLE, AGE_ACTIF, AGE_TAUX_ACTIF, AGE_DATE_CREATION, AGE_DATE_MAJ) VALUES
...
lock table banque2.clients write;
...
create table COMPTE(
    ...
    constraint FK_COM_AGE foreign key (AGE_ID) references AGENCE(AGE_ID)
);

lock table banque2.compte write;
INSERT INTO COMPTE (COM_SOLDE, COM_DATE_CREATION, COM_DATE_MAJ, CLI_ID, AGE_ID) VALUES
...
create table EMPRUNT(
    ...
    constraint FK_EMP_AGE foreign key (AGE_ID) references AGENCE(AGE_ID)
);

lock table banque2.emprunt write;
INSERT INTO EMPRUNT (EMP_MONTANT, EMP_DATE_CREATION, EMP_DATE_MAJ, CLI_ID, AGE_ID) VALUES
...
insert into audit_comptes values (1,0,0,0);

drop trigger comptes_inserees_trigger;
delimiter &&
create trigger comptes_inserees_trigger
after insert on emprunt
for each row
begin
    update audit_comptes set insertion =insertion+1 where id=1;
end&&
delimiter ;


drop trigger comptes_supprimer_trigger;
delimiter &&
create trigger comptes_supprimer_trigger
	after delete
on emprunt for each row 
begin
	update audit_comptes set supression = supression+1 where id=1;
    end&&
delimiter ;

drop trigger comptes_modifiees_trigger;
delimiter &&
create trigger comptes_modifiees_trigger
	after update
on emprunt for each row 
begin
	update audit_comptes set modification = modification+1 where id=1;
    end&&
delimiter ;

insert into compte values(null,1000,now(),now(),1,1);