...
lock tables banque.agence write;
...
lock table banque.clients write;
...
lock table banque.compte write;
...
lock table banque.emprunt write;
...
drop trigger emprunts_supprimees_trigger;
delimiter &&
create trigger emprunts_supprimees_trigger
	after delete
on emprunt for each row 
begin
insert into poubelle(concatener) values (concat(old.emp_id,' | ' , old.emp_montant, ' | ' , old.emp_date_creation, ' | ' , old.emp_date_maj, ' | ' , old.cli_id, ' | ' , old.age_id));
end&&
delimiter ;

delete from emprunt where emprunt.emp_id = 2;

create table if not exists poubelle (id int auto_increment, concatener varchar(500), primary key(id));
